...
void tshirts(int num){
    ...
    printf(ANSI_RED"%s exited,thread no %d \n",musician_thread[num]->name,num);
            ...
            { 
                if(musician_thread[musician_thread[num]->child]->status==0 && musician_thread[num]->status==0)
                {
                    ...
                    {   
                        ...
                        while(musician_thread[num]->status!=2) //change 4
                        ;
        ...
        {   
            ...
            { 
                if(musician_thread[musician_thread[num]->child]->status==0 && musician_thread[num]->status==0)
                {
                    ...
                    printf(ANSI_YELLOW"%s is performing %c at acoustic stage no. %d for %d sec\n",musician_thread[num]->name,musician_thread[num]->instru_char,musician_thread[num]->sem_no,time1);
                    sleep(time1);
                    if(musician_thread[num]->signal==1 && musician_thread[musician_thread[num]->singerno]->status!=2)
                    {
                       musician_thread[num]->singer=1;
                       sleep(2);  
                    }
                    if( musician_thread[num]->singer==1) //change 2
                    {   
        ...
        if(e==0)
        {
            ...
            else
            {
                ...
                if(musician_thread[musician_thread[num]->child]->status==0 && musician_thread[num]->status==0)
                {
                    ...
                    printf(ANSI_YELLOW"%s is performing %c at electric stage no.%d for %d sec\n",musician_thread[num]->name,musician_thread[num]->instru_char,musician_thread[num]->sem_no,time1);
                    sleep(time1);
                    if(musician_thread[num]->signal==1 && musician_thread[musician_thread[num]->singerno]->status!=2 ) //change 2
                    {
                       musician_thread[num]->singer=1;
                       sleep(2);  
                    }
                    if( musician_thread[num]->singer==1 && musician_thread[musician_thread[num]->singerno]->status!=2)
                    {                                                            //change 3           
                        printf(ANSI_GREEN"%s performance along with singer %s at electric stage no %d ended\n",musician_thread[num]->name,musician_thread[musician_thread[num]->singerno]->name,musician_thread[num]->sem_no);
                        musician_thread[musician_thread[num]->singerno]->status=2; //make singer status performed
